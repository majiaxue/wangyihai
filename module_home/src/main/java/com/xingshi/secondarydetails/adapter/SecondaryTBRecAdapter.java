...
public class SecondaryTBRecAdapter extends MyRecyclerAdapter<TBGoodsRecBean.ResultListBean> {
    ...
    public void convert(final RecyclerViewHolder holder, TBGoodsRecBean.ResultListBean data, int position) {
        ...
        if ("0".equals(data.getUser_type())) {
            //淘宝
            holder.setImageResource(com.xingshi.module_base.R.id.base_type, com.xingshi.module_base.R.drawable.taobao);
        } else {
            //天猫
            holder.setImageResource(com.xingshi.module_base.R.id.base_type, com.xingshi.module_base.R.drawable.tianmao);
        }
        ...
        holder.setImageFresco(com.xingshi.module_base.R.id.base_image, data.getPict_url());

        holder.setText(com.xingshi.module_base.R.id.base_name, data.getTitle());
        holder.setText(com.xingshi.module_base.R.id.base_reduce_price, "领劵减" + (data.getCoupon_amount() == null ? "0" : data.getCoupon_amount()) + "元");
        holder.setText(com.xingshi.module_base.R.id.base_preferential_price, "￥" + ArithUtil.sub(Double.valueOf(data.getZk_final_price() == null ? "0.0" : data.getZk_final_price()), Double.valueOf(data.getCoupon_amount() == null ? "0.0" : data.getCoupon_amount())));
        holder.setText(com.xingshi.module_base.R.id.base_original_price, "￥" + data.getZk_final_price());
        holder.setText(com.xingshi.module_base.R.id.base_number, "已抢" + data.getVolume() + "件");
...
//            LogUtil.e("预估收益：" + "个人收益" + SPUtil.getFloatValue(CommonResource.BACKBL) + "商品佣金" + div + "商品价格" + sub + "最终收益" + ArithUtil.mul(mul, SPUtil.getFloatValue(CommonResource.BACKBL)));
        ...
        holder.setText(com.xingshi.module_base.R.id.base_upgrade, "升级赚" + ArithUtil.mulRound(mul, 0.8));
