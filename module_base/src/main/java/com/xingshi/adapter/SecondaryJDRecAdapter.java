...
public class SecondaryJDRecAdapter extends MyRecyclerAdapter<JDListBean.DataBean> {
    ...
    public void convert(RecyclerViewHolder holder, JDListBean.DataBean data, int position) {
        ...
        if (!TextUtils.isEmpty(SPUtil.getToken())) {
            holder.setText(com.xingshi.module_base.R.id.base_estimate, "预估赚" + ArithUtil.mul(Double.valueOf(data.getCommissionInfo().getCommission()), SPUtil.getFloatValue(CommonResource.BACKBL)));
        } else {
            holder.setText(com.xingshi.module_base.R.id.base_estimate, "预估赚" + ArithUtil.mul(Double.valueOf(data.getCommissionInfo().getCommission()), 0.3));
        }
